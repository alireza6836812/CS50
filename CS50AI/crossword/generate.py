...
class CrosswordCreator:
    ...
    def enforce_node_consistency(self):
        ...
        """
        for var_name, word_list in self.domains.items():
            for word in word_list.copy():
                if len(word) != var_name.length:
    ...
    def order_domain_values(self, var: Variable, assignment: Assignment) -> list[str]:
        ...
        for word in self.domains[var]:
            count = 0
            for i in neighbours:
                overlapvar, overlapneighbour = self.get_definite_overlap(
                    var, i
                )

                for wordneighbour in self.domains[i]:
                    if word[overlapvar] != wordneighbour[overlapneighbour]:
                        count += 1

            output.append((count, var))
